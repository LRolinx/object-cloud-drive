...
-->
<template>
  <div class="body" v-if="props.uploadBufferPool.length!=0">
    <div :class="{minimize:isMinimize}" class="upLoadListBox" ref="upLoadListBox">
      <div class="head" :style="{flex:isMinimize?1:0}" @click="isMinimize && minimizeOrMaximize()">
        <div class="head_left">
          ...
          <p class="headLeftP">正在上传·剩余{{props.uploadRemainingTask}}项</p>
        </div>
      ...
      <ul class="upLoadListUlBox" v-show="!isMinimize">
        <li class="upLoadListLiBox" v-for="(item,index) in props.uploadBufferPool" :key="index">
...
const minimizeOrMaximize = () => {
  isMinimize.value = isMinimize ? false : true;
};
 ...
 * 上传文件状态颜色
 */
const upLoadTypeColor = (value: number) => {
  return value == 0
    ? ""
    : value == 1
    ? ""
