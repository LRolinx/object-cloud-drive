  ...
  <div class="driveBody" @mouseup="driveBodyMouseup" @dragenter.prevent.stop="dragenter" @mouseup.stop="fileBoxMouseup($event,null)" @contextmenu.prevent="">
    ...
    <div class="toolbar">
      <div class="toolbarLeft">
        <p class="toolbarTitle" :class="{colorR:store.drive.navigation.length >= 1}">{{store.drive.navigation.length ==
        0?`我的云盘(${props.fileData.length})`:'我的云盘'}}</p>
        <div v-for="(item,index) in store.drive.navigation" :key="item.id" style="display:inline-flex" :class="{toolbarOn:index!=store.drive.navigation.length-1}">
          ...
          <p class="toolbarTitle" :class="{colorR:index!=store.drive.navigation.length-1}">{{item.text}}<span v-if="index==store.drive.navigation.length-1">({{props.fileData.length}})</span></p>
        </div>
      </div>
...
const getFileFromEntryRecursively = async (folderId: string, entry: any) => {
  if (entry.isFile) {
    ...
    //检测到文件夹
    let createfolderres = await $http.post(
...
const getFileData = () => {
  ...
  const rowNumReal =
    props.fileData.length / colNum > rowNum ? rowNum + 1 : rowNum;
