...
<template>
  <div class="driveBody" @mouseup="driveBodyMouseup" @dragenter.prevent.stop="dragenter" @mouseup.stop="fileBoxMouseup($event, null)" @contextmenu.prevent="">
    <div class="draging" v-if="props.isShowUpdateModel" @dragover.prevent.stop="dragover" @dragleave.prevent.stop="dragleave" @drop.prevent.stop="drop">
    ...
    <div class="toolbar">
      <div class="toolbarLeft">
        <p class="toolbarTitle" :class="{ colorR: store.drive.navigation.length >= 1 }">{{ store.drive.navigation.length == 0 ? `我的云盘(${props.fileData.length})` : '我的云盘' }}</p>
        <div v-for="(item, index) in store.drive.navigation" :key="item.id" style="display: inline-flex" :class="{ toolbarOn: index != store.drive.navigation.length - 1 }">
          ...
          <p class="toolbarTitle" :class="{ colorR: index != store.drive.navigation.length - 1 }">
            {{ item.text }}<span v-if="index == store.drive.navigation.length - 1">({{ props.fileData.length }})</span>
          </p>
        </div>
      </div>
    ...
    <div class="dropZone" ref="recycleScroller">
      <!-- {{props}} -->
      <div v-if="props.fileData.length == 0" class="emptyBox">
        <img src="../static/img/empty.png" draggable="false" />
        <p>这里啥也没有呢~</p>
      </div>

      <div class="file-container" v-if="props.fileData.length != 0">
        <DynamicScroller :items="getFileData()" class="scroller" :min-item-size="1" key-field="key" v-slot="view">
          {{ view }}

          <div v-for="item in view.fileData" :key="item.id" class="fileBox" @dblclick="openFileOrFolder(item)" @mouseup.stop="fileBoxMouseup($event, item)" @contextmenu.prevent="">
    ...
    <div class="fileMenu" :style="{ top: `${props.fileMenuPos.y}px`, left: `${props.fileMenuPos.x}px` }" v-if="props.isShowRightMenu" @contextmenu.prevent="" @mouseup.stop="">
      <!-- 文件 -->
      <ul>
        <li v-if="props.showRightMenuType == 'default'" @click="openNewFolderModel">
          <i class="iconfont iconfont icon-add-folder"></i>
          <p>新建文件夹</p>
        </li>
        <li v-if="props.showRightMenuType == 'file'">
          <i class="iconfont icon-download"></i>
          <p>下载</p>
        </li>
        <li v-if="props.showRightMenuType == 'file' || props.showRightMenuType == 'folder'">
        ...
        <li v-if="props.showRightMenuType == 'file' || props.showRightMenuType == 'folder'" @click="showdelDialog">
    ...
    <!-- 新建模态窗 -->
    <newFile @oneFile="oneFile" v-model:isShowNewFileModel="props.isShowNewFileModel"></newFile>
    <!-- 新建文件夹模态窗 -->
    <newFolder v-model:isShowNewFolderModel="props.isShowNewFolderModel" @changeValue="addUserFolder"></newFolder>
    <!-- 视频模态窗 -->
...
const props: propsType = reactive({
  ...
  rightMenuItem: { type: '', id: 'string' }, //右键对应的Item对象
  fileData: [], //用户数据
...
const getFolderId = () => {
  ...
  folderid = typeof folderid == 'string' ? proxy.$route.params.folderId : proxy.$route.params.folderId[0]
  return folderid
}
  ...
  $http
    ...
    .then((res) => {
      if (res.data.code == 200) {
        ...
        props.fileData = res.data.data

        console.log(props.fileData, res.data.data)

        for (let i = 0; i < res.data.data.length; i++) {
          if (res.data.data[i].type == 'file' && checkType(res.data.data[i]).type == 'image') {
            ImageToblobUrl(i)
          }
          if (res.data.data[i].type == 'file' && checkType(res.data.data[i]).type == 'video') {
 ...
 * @param value
 */
const addUserFolder = (value: string) => {
  $http
...
const fileBoxMouseup = (e: any, item: any) => {
  props.isShowRightMenu = false
  props.showRightMenuType = item === null ? 'default' : item.type

  props.rightMenuItem = item
  //右点击文件与文件夹
  if (e.button == 2) {
    //右键
    props.fileMenuPos.x = e.x
    props.fileMenuPos.y = e.y
    // $set(, "x", e.x);
    // $set(, "y", e.y);
    props.isShowRightMenu = true
  }
}
...
const getFileFromEntryRecursively = async (folderId: string, entry: any) => {
  if (entry.isFile) {
    ...
    //检测到文件夹
    let createfolderres = await $http.post(`${store.serve.serveUrl}drive/addUserFolder`, {
      userid: store.id,
      folderid: folderId == '0' ? getFolderId() : folderId,
      name: entry.name,
    })
...
const openNewFolderModel = () => {
  // 显示新建文件夹模态窗
  props.isShowRightMenu = false
  props.isShowNewFolderModel = true
}
...
const showdelDialog = () => {
  props.isShowRightMenu = false
  if (props.rightMenuItem.type == 'file') {
    //删除文件
    $dialogMessge({
      title: '确定删除',
      text: `确定删除该文件嘛?`,
...
const delFileOrFolder = () => {
  ...
  $http
    .post(`${store.serve.serveUrl}drive/delUserFileOrFolder`, {
      id: props.rightMenuItem.id,
      type: props.rightMenuItem.type,
    })
    .then((res) => {
      if (res.data.code == 200) {
        //刷新
...
const getFileData = () => {
  //根据屏幕宽度计算可以显示多少个为一行
  const colNum = Number(Math.abs(props.width / 190))
  let arr = []
  //根据返回的数据可以显示几行
  const rowNum = Number(Math.abs(props.fileData.length / colNum))
  //根据计算检查是否满足一行，不够一行加一行
  const rowNumReal = props.fileData.length / colNum > rowNum ? rowNum + 1 : rowNum

  for (let i = 0; i < rowNumReal; i++) {
    arr.push([] as FileAndFloderType[])
    for (let j = i * colNum; j < i * colNum + colNum; j++) {
      arr[i].push(props.fileData[j])
    }
  }
...
watch(
  ...
  () => {
    //监听路由变化
    // console.log(to,from);
    // console.log(store.drive.currenFolderId,getFolderId())
    getUserFileAndFolder(getFolderId())
    let currentFolderList = toRaw(store.drive.navigation)
