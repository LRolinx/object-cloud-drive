  ...
  setup() {
    ...
    const login = () => {
      ...
      if (data.account == '' || data.account == null) {
        $tipMessge.open('用户名不能为空')
        return
      }
      if (data.password == '' || data.password == null) {
        $tipMessge.open('密码不能为空')
        return
      }
    ...
    const registered = () => {
      ...
      if (data.confirmPassword == '') {
        $tipMessge.open('确认密码不能为空')
        return
      }
      if (data.registeredCode == '') {
        $tipMessge.open('注册码不能为空')
        return
      }
      $http
        .post(`${store.serve.serveUrl}user/objectCloudDiskRegistered`, {
...
.switch {
  ...
  height: 20px;
  border-radius: 25px;
  box-shadow: 1px 2px 2px 0 rgba(0, 0, 0, 0.05), 0 2px 2px 0 rgba(0, 0, 0, 0.1), 0 3px 3px 0 rgba(0, 0, 0, 0.05);
  transition: ease-in 0.2s;
}
